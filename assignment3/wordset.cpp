...
namespace cs3505
{
  ...
  /** Constructor:  The parameter indicates the
    *    of the hashtable that should be used
    *   to keep this set.
    */
  ...
  void wordset::remove (std::string element)
  {
    ...
    while (current != NULL)
    {
      ...
      {        
        ...
        {
          table[index] = current->next;  // table element becomes next node
        } else { 
          last->next = current->next;  // remove from current list
        }
    ...
    return; //if words not in list do nothing
  }
  ...
  wordset & wordset::operator= (const wordset & rhs)
  {
    ...
    //change local variables and make a new table 
    this->capacity = rhs.capacity;
    this->count = 0;
    this->table = new node*[rhs.capacity];  // Create table (array of node pointers)
    ...
    //loop through old table and copy contents into new table 
    for(int i = 0; i < rhs.capacity; i++ )//loop through table
    {
      node *current_old = rhs.table[i];//current node in old table
      while(current_old != NULL)//loop through linked list
      {
         add(current_old->data);
  ...
  {
    int copied = 0; //the number of copied words
    //loop through table and add each element into the array  
    for(int i = 0; i < this->capacity; i++ )//loop through table
    {
      node *current_old = this->table[i];//current node in our table
      while(current_old != NULL)//loop through linked list
      {
         array[copied]=current_old->data;//add data into position copied then increment copied
         copied++;
         if(copied == n)//check if n words have been added 
           return; 
         current_old = current_old->next;
      }
    }
